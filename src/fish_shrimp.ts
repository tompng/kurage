...
function createGeometry([up, down]: [XY[], XY[]], wfunc: (l: number, t: number) => number) {
  const lines: XYZ[][] = [[], [], [], []]
...
export function createFishGeometryMaterial() {
  ...
  ctx.beginPath()
  ctx.arc(0.2, 0.8, 0.05, 0, 2 * Math.PI)
  ctx.fillStyle = '#444'
  ctx.fill()
  ctx.fillStyle = '#444'
...
export function createShrimpGeometryMaterial() {
  const geometry = createGeometry(shrimpOutline, l => l / 3)
  const [canvas, ctx] = createPaint()
  ctx.fillStyle = '#822'
  ctx.fillRect(0, 0, 1, 1)
  ctx.beginPath()
